...
| 14 | Padded torch `AndOr` uses more memory than the live graph. C board rows were a frozen JSONL from the C binary, not the Python wrapper. | spec | `NativeTypeNN` wraps C; `bench.py` `c-type-nn` / `c-type-nn-overfit` write board ids `type-nn` / `type-nn-overfit` with `backend=nativetypenn`. C sources live in `src/torch_type_nn/native/c` (package data; no hatch `force-include`). | `tests/test_native.py`, `test_c_board_rows_are_nativetypenn_impl_ids` |
